...
class System:
    def __init__(self, graph_model=None, architecture=None, additive=None, initial_conditions=None, simulation_parameters=None):
        ...
        self.simulation_parameters = {'T_sim': 200, 'T_predict': 30, 'model': None}
        ...
        self.trajectory = {'X0': 10*np.identity(self.dynamics['number_of_nodes']),
                           ...
                           'P': {}, 'E': {}, 'P_hist': [], 'E_hist': [], 'Z': [], 'compute_time': []}
    ...
    def model_rename(self, name_append=None):
        ...
        if self.simulation_parameters['model'] is not None:
            self.model_name = self.model_name + "_" + self.simulation_parameters['model']
    ...
    def plot_computetime(self, f_name=None, ax_in=None, plt_map=None, model='fixed'):
        if plt_map is None:
            plt_map = {'fixed': {'c': 'C0', 'line_style': 'solid', 'alpha': 0.5, 'zorder': 1},
                       'tuning': {'c': 'C1', 'line_style': 'solid', 'alpha': 0.5, 'zorder': 2},
                       'marker': {'B': "+", 'C': "+"}}
        ...
        if ax_in is None:
            if f_name is None:
                f_name = image_save_folder_path + self.model_name + '_computetime.png'
            plt.savefig(f_name)
            plt.show()
...
def greedy_architecture_selection(sys, number_of_changes=None, policy="min", no_select=False, print_check=False, t_start=time.time(), design_cost=False):
...
def greedy_architecture_rejection(sys, number_of_changes=None, policy="min", no_reject=False, print_check=False, t_start=time.time(), design_cost=False):
    if not isinstance(sys, System):
        raise Exception('Check data type')
    if print_check:
        print('\nRejection\n')
...
def greedy_simultaneous(sys, iterations=None, changes_per_iteration=1, fixed_set=None, failure_set=None, policy="min", t_start=time.time(), print_check=False, design_cost=False):
    if not isinstance(sys, System):
    ...
    while not iteration_stop:
        ...
        # Select one
        select = greedy_architecture_selection(work_iteration, number_of_changes=changes_per_iteration, policy=policy, t_start=t_start, no_select=True, print_check=print_check, design_cost=design_cost)
        ...
        # Reject one
        reject = greedy_architecture_rejection(work_iteration, number_of_changes=changes_per_iteration, policy=policy, t_start=t_start, no_reject=True, print_check=print_check, design_cost=design_cost)
...
def simulate_fixed_architecture(S, print_check=True, multiprocess_check=False):
    ...
    if multiprocess_check:
        ...
        for t in tqdm(range(0, T_sim), ncols=100, position=process_number, leave=False):
            ...
            S_fixed.trajectory['compute_time'].append(time.time() - t_start)
    else:
...
def simulate_selftuning_architecture(S, iterations_per_step=1, changes_per_iteration=1, print_check=True, multiprocess_check=False):
    ...
    if multiprocess_check:
        ...
        for t in tqdm(range(0, T_sim), ncols=100, position=process_number, leave=False):
            ...
            S_tuning.trajectory['compute_time'].append(time.time() - t_start)
    else:
        for t in tqdm(range(0, T_sim), ncols=100, leave=False):
            t_start = time.time()
            S_tuning.cost_wrapper_enhanced_true()
...
def greedy_architecture_initialization(S, print_check=False):
    ...
    S_test.simulation_parameters['T_predict'] *= 5
...
def time_axis_plot(model):
    ...
    grid_timeplots = grid_outer[0, 0].subgridspec(6, 1, hspace=0)
    ...
    ax_computetime = fig.add_subplot(grid_timeplots[5, 0], sharex=ax_cost)
    ...
    S_fixed.plot_computetime(ax_in=ax_computetime, model='fixed')
    S_tuning.plot_computetime(ax_in=ax_computetime, model='tuning')
...
def statistics_plot(test_model, n_samples=100):
    ...
    eig_val_plot = fig.add_subplot(grid[2:, 0])
    arch_scatter_plot = fig.add_subplot(grid[2:, 1])
    compute_time_plot = fig.add_subplot(grid[1, :])
