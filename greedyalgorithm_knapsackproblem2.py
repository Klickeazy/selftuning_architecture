...
class ItemList:
    ...
    def display_list(self):
        ...
        print('IDs: ', [i.item_id for i in self.items])
        print('Values: ', [i.item_value for i in self.items])
...
def greedy_selection(total_set, work_set, limit, number_of_changes=None, exclusion_set=None, max_greedy_limit=max_limit, min_greedy_limit=min_limit, cost_metric=knapsack_value, policy="max", t_start=time.time(), no_select=False, status_check=False):
    if not isinstance(total_set, ItemList) or not isinstance(work_set, ItemList) or not (isinstance(exclusion_set, ItemList) or exclusion_set is None):
        raise Exception('Incorrect data type')
    ...
    while max_greedy_limit(work_iteration, limit[1]):
        ...
        choice_iteration = total_set.compare_lists(work_iteration)['unique']
        if exclusion_set is not None:
            choice_iteration = exclusion_set.compare_lists(choice_iteration)['absent']
    ...
    return {'work_set': work_iteration, 'work_history': work_history, 'choice_history': choice_history, 'time': time.time()-t_start}


def greedy_rejection(total_set, work_set, limit, number_of_changes=None, exclusion_set=None, max_greedy_limit=max_limit, min_greedy_limit=min_limit, cost_metric=knapsack_value, policy="max", t_start=time.time(), no_reject=False, status_check=False):
    if not isinstance(total_set, ItemList) or not isinstance(work_set, ItemList) or not (isinstance(exclusion_set, ItemList) or exclusion_set is None):
        raise Exception('Incorrect data type')
...
def greedy_simultaneous(total_set, work_set, limit, iterations=1, changes_per_iteration=1, exclusion_set=None, max_greedy_limit=max_limit, min_greedy_limit=min_limit, cost_metric=knapsack_value, policy="max", t_start=time.time(), status_check=False):
    if not isinstance(total_set, ItemList) or not isinstance(work_set, ItemList) or not (isinstance(exclusion_set, ItemList) or exclusion_set is None):
        raise Exception('Incorrect data type')
    ...
    for _ in range(0, iterations):
        ...
        # Select one
        select = greedy_selection(total_set, work_iteration, limit, number_of_changes=changes_per_iteration, exclusion_set=exclusion_set, max_greedy_limit=max_greedy_limit, min_greedy_limit=min_greedy_limit, cost_metric=cost_metric, policy=policy, no_select=True, status_check=status_check)
        iteration_cases.append(select['work_set'])
        values.append(cost_metric(iteration_cases[-1]))

        # Reject one
        reject = greedy_rejection(total_set, work_iteration, limit, number_of_changes=changes_per_iteration, exclusion_set=exclusion_set, max_greedy_limit=max_greedy_limit, min_greedy_limit=min_greedy_limit, cost_metric=cost_metric, policy=policy, no_reject=True, status_check=status_check)
        iteration_cases.append(reject['work_set'])
        values.append(cost_metric(iteration_cases[-1]))

        # Swap: add then drop
        swap_select1 = greedy_selection(total_set, work_iteration, limit+np.array([0, 1]), number_of_changes=changes_per_iteration, exclusion_set=exclusion_set, max_greedy_limit=max_greedy_limit, min_greedy_limit=min_greedy_limit, cost_metric=cost_metric, policy=policy, no_select=True, status_check=False)
        swap_reject1 = greedy_rejection(total_set, swap_select1['work_set'], limit, number_of_changes=changes_per_iteration, exclusion_set=exclusion_set, max_greedy_limit=max_greedy_limit, min_greedy_limit=min_greedy_limit, cost_metric=cost_metric, policy=policy, no_reject=True, status_check=status_check)
        iteration_cases.append(swap_reject1['work_set'])
        values.append(cost_metric(iteration_cases[-1]))

        # # Swap: drop then add
        # swap_reject2 = greedy_rejection(total_set, work_iteration, limit-np.array([1, 0]), exclusion_set=exclusion_set, max_greedy_limit=max_greedy_limit, min_greedy_limit=min_greedy_limit, cost_metric=cost_metric, policy=policy, no_reject=True, status_check=False)
        # swap_select2 = greedy_selection(total_set, swap_reject2['work_set'], limit, exclusion_set=exclusion_set, max_greedy_limit=max_greedy_limit, min_greedy_limit=min_greedy_limit, cost_metric=cost_metric, policy=policy, no_select=True, status_check=status_check)
    ...
    return {'work_set': work_iteration, 'work_history': work_history, 'time': time.time()-t_start}
