...
class Experiment:
    def __init__(self):
        ...
        self.parameter_keys = list(self.default_parameter_datatype_map.keys())
        self.parameter_dtypes = {k: type(self.default_parameter_datatype_map[k]) for k in self.default_parameter_datatype_map}
...
def test_all_experiments():
    print('Testing initializing all experiment models')
    Exp = Experiment()
...
class System:
            ...
            for t in range(0, self.sim.t_simulate, self.disturbance.disturbance_step):
                if self.disturbance.noise_model in ['process', 'combined']:
                    self.disturbance.w_gen[t][
                        np.random.default_rng().choice(self.number_of_nodes, self.disturbance.disturbance_number, replace=False)] = self.disturbance.disturbance_magnitude * np.array(
    ...
    def prediction_control_gain(self, update_trajectory_check=False):
        ...
        for t in range(self.sim.t_predict - 1, -1, -1):
            self.B.gain[t] = np.linalg.inv((self.B.active_matrix.T @ self.B.recursion_matrix[t + 1] @ self.B.active_matrix) + self.B.R1) @ self.B.active_matrix.T @ self.B.recursion_matrix[
                t + 1] @ self.A.A_mat
            self.B.recursion_matrix[t] = (self.A.A_mat.T @ self.B.recursion_matrix[t + 1] @ self.A.A_mat) - (
                        self.A.A_mat.T @ self.B.recursion_matrix[t + 1] @ self.B.active_matrix @ self.B.gain[t]) + self.B.Q
    ...
    def prediction_estimation_gain(self, update_trajectory_check=False):
        ...
        for t in range(0, self.sim.t_predict):
            ...
            self.C.recursion_matrix[t + 1] = (self.A.A_mat @ self.C.recursion_matrix[t] @ self.A.A_mat.T) - (
                        self.A.A_mat @ self.C.gain[t] @ self.C.active_matrix @ self.C.recursion_matrix[t] @ self.A.A_mat.T) + self.C.Q
        if update_trajectory_check:
    ...
    def cost_architecture_switching(self):
        if self.trajectory.cost.metric_switching == 0 or (self.B.R3 == 0 and self.C.R3 == 0):
            ...
            self.trajectory.cost.switching = np.linalg.norm(self.B.indicator_vector_current - self.B.indicator_vector_history) * self.B.R3 + np.linalg.norm(
                self.C.indicator_vector_current - self.C.indicator_vector_history) * self.C.R3
            ...
            self.trajectory.cost.switching = (self.B.indicator_vector_current - self.B.indicator_vector_history).T @ self.B.R2 @ (self.B.indicator_vector_current - self.B.indicator_vector_history) + (
                        self.C.indicator_vector_current - self.C.indicator_vector_history).T @ self.C.R2 @ (self.C.indicator_vector_current - self.C.indicator_vector_history)
        else:
...
def arch_update_or_terminate(S, S_ref):
    ...
    if S.architecture_compare_active_set_to_system(S_ref):
        ...
        S.architecture_update_to_history_indicator_matrix_from_active_set()
